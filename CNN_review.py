...
train_x_review = X_review_train_array[:700]
train_y = y_train_array[:700]
test_x = X_image_test_array[:500]
test_x_review = X_review_test_array[:500]
test_y = y_test_array[:500]


# define accuracy
def compute_accuracy(v_xs, v_xv, v_ys):
    global prediction
    y_pre = sess.run(prediction, feed_dict={xs:v_xs, xv: v_xv, keep_prob: 1})
    correct_prediction = tf.equal(tf.argmax(y_pre, 1), tf.argmax(v_ys, 1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    result = sess.run(accuracy, feed_dict={xs: v_xs,xv: v_xv, ys:v_ys, keep_prob: 1})
    return result
...
xv = tf.placeholder(tf.float32, [None, 5])
...
h_fc_2_review = h_fc2 + xv
prediction = tf.nn.softmax(h_fc_2_review + b_fc2)
...
for i in range(1000):
    ...
    sess.run(train_step, feed_dict={xs: train_x, xv: train_x_review, ys: train_y, keep_prob: 0.5})
    # _, loss_val = sess.run([train_step, cross_entropy], feed_dict={xs: test_x, xv: test_x_review, ys: test_y, keep_prob: 0.5})
    if i % 50 == 0:
        # print("iteration", i, "accuracy", compute_accuracy(test_x,test_x_review, test_y), "loss", loss_val)
        print("iteration", i, "accuracy", compute_accuracy(test_x, test_x_review, test_y))